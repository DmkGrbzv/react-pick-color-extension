  ...
  const storage = new StorageAdapter( {
    local: {
      async get() {
        ...
        return { currentPalette: saved };
      },
      async set( value ) {
        if ( writeFails ) throw failure;
        saved = structuredClone( value.currentPalette );
      },
    },
  } );
  const service = new PaletteService( new PaletteRepository( storage ), () => 'color-id' );
...
test( 'message listener rejects wrong senders and unknown/null messages independently', () => {
  ...
  assert.equal(
    listener( { type: findCorrectMessageType( GET_PALETTE ) }, { id: 'other-id' }, respond ),
    false
  );
...
test( 'worker serializes async service errors and synchronous errors at one boundary', async () => {
  for ( const [failure, expected] of [
    ...
    const listener = createListener( 'own-id', () => {
      throw failure;
    } );
    const response = await new Promise( ( resolve ) => {
      assert.equal(
        listener( { type: findCorrectMessageType( GET_PALETTE ) }, { id: 'own-id' }, resolve ),
        true
      );
    } );
    assert.deepEqual( response, { ok: false, error: expected, details: failure.message } );
  }
  const listener = createListener( 'own-id', async () => {
    throw new Error( 'Window missing' );
  } );
  const response = await new Promise( ( resolve ) =>
    listener( { type: findCorrectMessageType( OPEN_EDITOR ) }, { id: 'own-id' }, resolve )
  );
...
test( 'router preserves sender window priority and rejects unsupported commands', async () => {
  ...
  await assert.rejects( dispatchMessage( handlers, null ), { code: 'unknownOperation' } );
} );
...
test( 'client rejects failures up to the UI and preserves their cause', async () => {
  ...
  try {
    ...
    await assert.rejects(
      sendRequest( {} ),
      ( error ) => error.code === 'extensionUnavailable' && error.cause === failure
    );
    ...
    await assert.rejects(
      sendRequest( {} ),
      ( error ) => error.code === 'storageFailure' && error.cause.message === 'Disk unavailable'
    );
