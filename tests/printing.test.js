...
test( 'all print layouts preserve mixed items and paginate at exact boundaries without blank trailing pages', () => {
  for ( const orientation of ['portrait', 'landscape'] )
    for ( const size of ['medium', 'large'] ) {
      ...
      const items = Array.from( { length: layout.capacity * 2 + 1 }, ( _, id ) => ( {
        id,
        type: id % 2 ? 'gradient' : 'color',
      } ) );
...
test( 'format preference persists and syncs without modifying palette storage', async () => {
    ...
    local: {
      ...
      async set( values ) {
        ...
        for ( const listener of listeners )
          listener( { colorFormat: { newValue: values.colorFormat } }, 'local' );
      },
    },
    onChanged: {
  ...
  const client = new ColorFormatPreference( api );
  assert.equal( await client.read(), 'hex' );
  const updates = [];
  const stop = new ColorFormatPreference( api ).subscribe( ( value ) => updates.push( value ) );
  await client.save( 'cmyk' );
  assert.equal( await new ColorFormatPreference( api ).read(), 'cmyk' );
...
test( 'format storage failures propagate to the caller', async () => {
  ...
  const client = new ColorFormatPreference( {
    local: {
      async get() {
        throw failure;
      },
...
test( 'print preview disables its own side panel without changing another tab', async () => {
  ...
  const api = {
    ...
    tabs: {
      async get( id ) {
        return { id, url: 'chrome-extension://test/print.html' };
      },
    },
    sidePanel: {
      async getOptions() {
    ...
    action: {
      ...
      async enable() {
        assert.fail( 'Unexpected action enable' );
      },
    },
  };
  await new PanelController( api ).sync( 8 );
